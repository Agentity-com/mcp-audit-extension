...
export function startRemoteMcpProxy() {
    ...
    */
   app.use('/.well-known/oauth-protected-resource/:server', (req, res) => {
       const resourceMetadata = {
           resource_name: req.params.server,
           resource: `http://127.0.0.1:${PORT}/${req.params.server}`,
           authorization_servers: [proxyConfig[req.params.server!].origin],
   ...
   app.use(
    ...
    function fixResponses(proxyRes: IncomingMessage, req: Request, res: Response) {
        ...
        Object.entries(proxyRes.headers)
            ...
            .forEach(([key, value]) => {
                if ((proxyRes.statusCode === 401) && (key === 'www-authenticate') && (!value?.includes('resource_metadata'))) {
                    // This is the case where server requires auth but doesn't supprot PRM per 2025-06-18 spec
